    ...
    apiSQL.query("SELECT * FROM `restkey` WHERE `email` = ?", email, function(err, result) {
        ...
        apiSQL.query("SELECT * FROM `restkey` WHERE `username` = ?", userName, function(err, result) {
            ...
            transporter.sendMail({
                from: `"Maceng Rest-API" <${process.env.mail_FROM}>`, // sender address
                to: `"${userName}" <${email}>`, // list of receivers
                subject: "Maceng | Email Verification", // Subject line
                text: "Maceng Rest-API", // plain text body
                html: `
                <p>Hallo ${firstName} ${lastName}</p></br>
                <p>Congratulations! You will soon bind this email address to your Maceng Rest-API account.</p></br>
                ...
                apiSQL.query("INSERT INTO `restkey` SET?", { updateAt: mysqlTimestamps, createdAt: mysqlTimestamps, firstname: firstName, lastname: lastName, username: userName, email: email, password: hashedPassword, secretToken: TokenNya, apiKey: '0'+ crypto.randomBytes(Math.floor(Math.random() * 2) +5).toString('hex'), recovery: 'success', active: 'false' }, function(err, result) {
                    if (err) return res.render('register', { message: "Error on database!", messageClass: 'red' })
	...
	if (req.body.email) {
		...
		apiSQL.query("SELECT * FROM `restkey` WHERE `email` = ?", req.body.email, function(err, result) {
			...
			transporter.sendMail({
				...
				subject: "Maceng | Password Recovery", // Subject line
				text: "Maceng Rest-API", // plain text body
				html: `
